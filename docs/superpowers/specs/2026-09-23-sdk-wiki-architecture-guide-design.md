...
1. **核心能力**
   - 先说明核心能力由平台客户端统一组装，业务代码通常不直接创建底层 core 对象。
   - 展示默认客户端已经启用的能力，以及适合直接使用默认值的场景。
   - 提供一段完整的生产环境组合示例，演示 HTTP Client、共享缓存、重试和观测 Hook 的注入方式。
   - 用表格列出每个 `core/*` 包解决的问题、平台客户端入口和需要直接配置的场景。
   - 按单实例、多实例、统一凭据、代理、重试、观测、回调和测试场景给出选择建议。
2. **整体架构**
