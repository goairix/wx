...
func (c *Client) CheckAppointmentLimit(
	ctx context.Context,
	req CheckAppointmentLimitRequest,
) (CheckAppointmentLimitResponse, error) {
	var out CheckAppointmentLimitResponse
	err := c.caller.Call(ctx, appointmentLimitPath, req, &out)
	return out, err
}

// CancelAppointmentLimit 通知腾讯用户已取消预约，用于完善防黄牛风控数据。
func (c *Client) CancelAppointmentLimit(
	ctx context.Context,
	req CancelAppointmentLimitRequest,
) (CancelAppointmentLimitResponse, error) {
