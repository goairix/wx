...
func (c *Client) NotifyReferralResult(
	ctx context.Context,
	req NotifyReferralResultRequest,
) (NotifyReferralResultResponse, error) {
	var out NotifyReferralResultResponse
	err := c.caller.Call(ctx, referralPath, req, &out)
	return out, err
}

// SendMedicalRecordNotice 向用户推送就医记录通知。
func (c *Client) SendMedicalRecordNotice(
	ctx context.Context,
	req SendMedicalRecordNoticeRequest,
) (SendMedicalRecordNoticeResponse, error) {
	var out SendMedicalRecordNoticeResponse
	err := c.caller.Call(ctx, medicalRecordPath, req, &out)
	return out, err
}
