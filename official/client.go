...
func NewClient(config Config, opts ...Option) (*Client, error) {
	if strings.TrimSpace(config.AppID) == "" || strings.TrimSpace(config.AppSecret) == "" {
		return nil, fmt.Errorf("official: AppID and AppSecret are required")
	}
	optsState := &option{}
	...
	client.auth = auth.NewManager("official", config.AppID, cache, auth.ProviderFunc(client.fetchToken))
